...
このファイルはこのリポジトリで gemili-cli を使用する際のガイダンスを提供します。
...
**DMPC Gemini CLIがlocal_nlgブランチにコミットした変更をNLGPCに反映する方法**:
...
**推奨運用フロー**:
1. DMPC Gemini CLIがNLG関連ファイルを編集 → local_nlgにコミット・プッシュ
...
- **NLGモジュール編集**: DMPC Gemini CLIが `local_nlg` ブランチに切り替えて編集・コミット
