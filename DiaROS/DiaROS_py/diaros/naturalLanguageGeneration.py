...
class NaturalLanguageGeneration:
    def __init__(self, dm_ref=None, rnlg_ref=None):
        ...
        # ★Second stage処理中の割り込み制御
        self.is_second_stage_processing = False  # Second stage処理中フラグ
        self.pending_first_stage_request = None  # Second stage処理中に来たfirst_stageリクエストを保持
    ...
    def update(self, words, stage='first', turn_taking_decision_timestamp_ns=0, first_stage_backchannel_at_tt=None, asr_history_2_5s=None):
        ...
        # ★【重要】Second stage処理中の割り込み制御
        # Second stage処理中にfirst_stageリクエストが来た場合、
        # 最新のfirst_stageリクエストだけ保持して、処理を中止する
        if self.is_second_stage_processing and stage == 'first':
            timestamp = now.strftime('%H:%M:%S.%f')[:-3]
            sys.stdout.write(f"[{timestamp}] [QUEUE] Second stage処理中のためfirst_stageをキューイング\n")
            sys.stdout.flush()
            # 最新のfirst_stageリクエストを保持
    ...
    def generate_second_stage(self, query):
        ...
        # ★Second stage処理開始フラグを設定
        self.is_second_stage_processing = True
        ...
        try:
            ...
            # ★Second stage処理終了フラグをリセット
            self.is_second_stage_processing = False

            # ★pending_first_stage_requestがあれば処理
            if self.pending_first_stage_request:
                timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
                sys.stdout.write(f"[{timestamp}] [DEQUEUE] Second stage完了、キューイングされたfirst_stageを処理します\n")
                sys.stdout.flush()

                pending_req = self.pending_first_stage_request
                self.pending_first_stage_request = None  # クリア

                # pending_first_stage_requestを処理（再帰的にupdate()を呼び出す）
                self.update(
                    words=pending_req['words'],
                    stage=pending_req['stage'],
