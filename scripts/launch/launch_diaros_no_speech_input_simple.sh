...
# DiaROS起動 (speech_inputノード除外・分散実行)
# ★デフォルトで分散実行モード（NLGノード除外）で起動
# NLGPC側で別途実行: ros2 run diaros_package ros2_natural_language_generation

NLG_PARAM="nlg:=false"  # ★デフォルトで分散実行モード
echo "⚙️  分散実行モード: NLGノードを除外します"
echo "📝 NLGPC側で以下を実行してください:"
